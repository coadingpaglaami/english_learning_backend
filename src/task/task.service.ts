...
export class TaskService {
  ...
  if (passageImage) {
    newPassageImageUrl = await this.uploadService.uploadSingleImage(passageImage);
  }
  ...
  return this.prisma.$transaction(async (tx) => {
    ...
    if (dto.updateWords?.length) {
      for (const word of dto.updateWords) {
        ...
        if (word.definition !== undefined) updateData.definition = word.definition;
        ...
        if (existingTask.readingContent) {
          await tx.readingTask.update({
            ...
            data: {
              ...
              entryType: dto.entryType ?? existingTask.readingContent.entryType,
              imageUrl:
                newPassageImageUrl !== undefined
                  ? newPassageImageUrl
                  : dto.removePassageImage
        ...
        if (existingTask.grammarContent) {
          await tx.grammarTask.update({
            ...
            data: {
              ...
              entryType: dto.entryType ?? existingTask.grammarContent.entryType,
            },
          });
        } else {
